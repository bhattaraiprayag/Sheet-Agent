...
def test_settings_missing_required_field(monkeypatch: "MonkeyPatch", tmp_path: "MagicMock") -> None:
    """
    Tests that a ValidationError is raised if a required field is missing.
    """
    get_settings.cache_clear()
    # Use a clean directory with no .env file so dotenv source doesn't interfere
    monkeypatch.chdir(tmp_path)
...
def test_gcp_secret_manager_loading(
    monkeypatch: "MonkeyPatch", mocker: "MockerFixture", tmp_path: "MagicMock"
) -> None:
    """
